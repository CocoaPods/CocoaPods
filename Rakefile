        ...
        Dir.chdir(dir) do
          ...
          workspace.schemes.each do |scheme_name, project_path|
            ...
            when :ios
              ...
              testCmd = (scheme_name.start_with? 'Test') ? 'test' : ''
              execute_command "xcodebuild -workspace '#{workspace_path}' -scheme '#{scheme_name}' clean build #{testCmd} ONLY_ACTIVE_ARCH=NO -destination 'platform=iOS Simulator,name=iPhone 6s'"
            else
