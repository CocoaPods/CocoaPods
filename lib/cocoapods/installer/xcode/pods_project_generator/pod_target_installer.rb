        ...
        class PodTargetInstaller < TargetInstaller
          ...
          def add_test_app_host_targets(test_native_targets, app_native_targets)
            target.test_spec_consumers.reject(&:requires_app_host?).select(&:app_host_name).each do |test_spec_consumer|
              raise "`#{target.label}-#{test_spec_consumer.test_type}-Tests` manually specifies an app host but has `test_spec.requires_app_host = false`! Please set `test_spec.requires_app_host = true`"
            end
            ...
            target.test_spec_consumers.select(&:requires_app_host?).select(&:app_host_name).each do |test_spec_consumer|
              ...
              raise 'No matching app_spec found' unless app_native_target
