        ...
        if installation_options.deduplicate_targets?
          distinct_targets = resolver_specs_by_target.each_with_object({}) do |dependency, hash|
            ...
            dependent_specs.group_by(&:root).each do |root_spec, resolver_specs|
              ...
              specs = all_specs.select(&:library_specification?)
              test_specs = all_specs.select(&:test_specification?)
              app_specs = all_specs.select(&:app_specification?)
              pod_variant = PodVariant.new(specs, test_specs, app_specs, target_definition.platform, target_definition.uses_frameworks?)
              hash[root_spec] ||= {}
              (hash[root_spec][pod_variant] ||= []) << target_definition
              variant = hash[root_spec].keys.find { |k| k == pod_variant }
              variant.test_specs.concat(test_specs).uniq!
              variant.app_specs.concat(app_specs).uniq!
            end
          end
