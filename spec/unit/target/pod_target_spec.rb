  ...
  describe PodTarget do
    ...
    describe 'Product type dependent helpers' do
      ...
      describe 'test spec support' do
        ...
        it 'returns the framework path of the library itself when requiring dynamic frameworks' do
          fa = Sandbox::FileAccessor.new(nil, @coconut_spec.consumer(@platform))
          @test_pod_target.stubs(:file_accessors).returns([fa])
          @test_pod_target.stubs(:requires_frameworks?).returns(true)
          @test_pod_target.stubs(:should_build?).returns(true)
          @test_pod_target.stubs(:static_framework?).returns(false)
          @test_pod_target.framework_paths.should == {
            'CoconutLib' => [
