    ...
    describe 'In general' do
      before do
        ...
        @target = AggregateTarget.new(config.sandbox, false, {}, [],
                                      Platform.ios, @target_definition, config.sandbox.root.dirname,
    ...
    describe 'Pod targets' do
      before do
        ...
        @pod_target = PodTarget.new(config.sandbox, false, {}, [],
                                    Platform.ios, [@spec], [@target_definition],
                                    [file_accessor])
        @target = AggregateTarget.new(config.sandbox, false, {}, [],
                                      Platform.ios, @target_definition, config.sandbox.root.dirname, nil, nil,
                                      [@pod_target])
      end
      ...
      describe 'with configuration dependent pod targets' do
        before do
          ...
          @pod_target_release = PodTarget.new(config.sandbox, false, {}, [],
                                              Platform.ios, [@spec], [@target_definition], [file_accessor])
