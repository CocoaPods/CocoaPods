Pod::Spec.new do |s|
  s.name         = 'BananaLib'
  s.version      = '1.1'
  s.authors      = 'Banana Corp', { 'Monkey Boy' => 'monkey@banana-corp.local' }
  s.homepage     = 'http://banana-corp.local/banana-lib.html'
  s.summary      = 'Chunky bananas!'
  s.description  = 'Full of chunky bananas.'
  s.source       = { :git => 'http://banana-corp.local/banana-lib.git', :tag => 'v1.1' }
  s.source_files = 'Classes/*.{h,m}', 'Vendor'
  s.xcconfig     = { 'OTHER_LDFLAGS' => '-framework SystemConfiguration' }
  s.prefix_header_file = 'Classes/BananaLib.pch'
  s.resources    = "Resources/*.png"
  s.requires_arc = true
  s.dependency   'monkey', '~> 1.0.1', '< 1.0.9'
  s.swift_versions = ['4.2', '5']
  s.license      = {
    :type => 'MIT',
    :file => 'LICENSE',
    :text => 'Permission is hereby granted ...'
  }

  s.test_spec 'UnitTests' do |test_spec|
    test_spec.source_files = 'Tests/Unit/*.{h,m,swift}'
  end

  s.test_spec 'SnapshotTests' do |test_spec|
    test_spec.requires_app_host = true
    test_spec.source_files = 'Tests/Snapshot/*.{h,m,swift}'
  end

  s.app_spec 'DemoApp' do |app_spec|
    app_spec.source_files = 'Example/*.{h,m,swift}'
  end
end
